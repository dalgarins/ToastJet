...
import com.ronnie.toastjet.engine.apiEngine.graphql.invokeGraphQL
...
data class PostGraphQLResponse(
    val res: GraphQLResponseData,
    val config: ConfigData
)

class GraphQLResponsePanel(private val store: GraphQLStore) : JPanel() {
    ...
    fun invoke() {
        configStore?.let { configStore ->
            coroutineScope.launch {
                val request = ScriptExecutor.executePrescriptCode<GraphQLRequestData>(
                    gson.toJson(
                        mapOf(
                            Pair("config", configStore.state.getState()),
                            Pair("api", store.getCurrentGraphQLFromStates())
                        )
                    )
                )
                ...
                withContext(Dispatchers.Swing) {
                    ...
                    SwingUtilities.invokeLater {
                        ...
                        revalidate()
                        repaint()
                        val reqRes = ScriptExecutor.executePostscriptCode<PostGraphQLResponse>(
                            gson.toJson(
                                mapOf(
                                    Pair("req", store.getCurrentGraphQLFromStates()),
    ...
    fun generatePanel() {
        ...
        if (store.response.getState().isBeingInvoked) {
            ...
            if (store.response.getState().invoked) {
                add(GraphQLResponseInvoked(store))
            } else {
